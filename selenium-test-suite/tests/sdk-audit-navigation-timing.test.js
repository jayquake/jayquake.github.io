/**
 * Navigation Timing Tests (Selenium)
 *
 * Mirrors the Playwright navigation-audit-timing.spec.js using Selenium WebDriver.
 */
...
describe(`Navigation Timing Tests (SDK ${ENABLE_SDK_AUDIT ? 'ENABLED' : 'DISABLED'})`, () => {
  beforeEach(async () => {
  ...
  test('should measure home page load and audit time', async () => {
    const pageMetrics = {
      path: '/',
      navigationTime: 0,
      auditTime: 0,
    ...
    const heading = await driver.wait(
      until.elementLocated(By.xpath('//*[contains(text(), "Rule Library")]')),
      10000,
    );
  ...
  test(
    ...
    for (const rulePage of rulePages) {
      ...
      const navStart = performance.now();
      await navigateTo(driver, rulePage.path);
  ...
  test(
    ...
    for (const enginePage of enginePages) {
      ...
      const navStart = performance.now();
      await navigateTo(driver, enginePage.path);
      await driver.sleep(300);
      pageMetrics.navigationTime = performance.now() - navStart;
  ...
  test(
    ...
    for (const pagePath of allPages) {
      const navStart = performance.now();
      await navigateTo(driver, pagePath);
      await driver.sleep(300);
      ...
      console.log(
        `   ${pagePath.padEnd(45)} | Nav: ${formatTime(navTime).padEnd(8)} | Audit: ${formatTime(auditTime)}`,
      );
    }
    ...
    console.log(
      `   Total Navigation Time: ${formatTime(comprehensiveResults.totalNavTime)}`,
    );
    console.log(
    ...
    console.log(
      `   Average Audit Time/Page: ${formatTime(comprehensiveResults.avgAuditTime)}`,
    );
    console.log(
      `   Total Time: ${formatTime(comprehensiveResults.totalNavTime + comprehensiveResults.totalAuditTime)}`,
    );
    console.log('='.repeat(60));
