...
describe("TabListMisuse Rule Validation", () => {
  ...
  it("should have no failed nodes when tab-list has 'tablist' role", async () => {
    ...
    await TabListMisuse.validate(validateMethodArguments);

    expect(response.failedNodes).toHaveLength(0);
  });

  it("should have failed nodes when non-tab-list element has 'tablist' role", async () => {
    const { response, document, classifier } = validateMethodArguments;
    const element = document.createElement("div");
    (classifier.getMatched as jest.Mock).mockReturnValue([element]);
